...
cities.rename(mapper=str.strip,axis='columns')
...
def nhl_correlation(): 
    ...
    result = pd.merge(nhl_df,cities, left_on= "team_ville", right_on= "Metropolitan area")

    result['W'] = pd.to_numeric(result['W'])
    result['L'] = pd.to_numeric(result['L'])
    result['Population (2016 est.)[8]'] = pd.to_numeric(result['Population (2016 est.)[8]'])

    he = ['team','W','L','Metropolitan area','Population (2016 est.)[8]']

    result = result[he]

    result['w/l'] = result['W']/(result['L']+result['W'])

    result = result.groupby('Metropolitan area').mean().reset_index()
...
def nba_correlation():
    ...
    for i in nba_df['team']:
        #i=i.rstrip()
        i=i.split('*')
        team_spliter1.append(i[0])
    nba_df['team'] =team_spliter1
    
    team_spliter2 = []
    for i in nba_df['team']:
        i=i.split('(')
        team_spliter2.append(i[0])
    nba_df['team'] = team_spliter2
    
    team_spliter3 = []
    for i in nba_df['team']:
        i=i.rstrip()
        team_spliter3.append(i)
    nba_df['team'] = team_spliter3
    ...
    result2 = pd.merge(nba_df,cities, left_on= "team_ville", right_on= "Metropolitan area")
    
    result2['W/L%'] = pd.to_numeric(result2['W/L%'])
    result2['W'] = pd.to_numeric(result2['W'])
    result2['L'] = pd.to_numeric(result2['L'])
    result2['Population (2016 est.)[8]'] = pd.to_numeric(result2['Population (2016 est.)[8]'])
    he = ['team','W','L','W/L%','Metropolitan area','Population (2016 est.)[8]']
    result2 = result2[he]
    result2['w/l'] = result2['W']/(result2['L']+result2['W'])
    result2 = result2.groupby('Metropolitan area').mean().reset_index()
    
    population_by_region = result2['Population (2016 est.)[8]'] # pass in metropolitan area population from cities
    win_loss_by_region = result2['w/l'] # pass in win/loss ratio from nba_df in the same order as cities["Metropolitan area"]
...
def mlb_correlation(): 
    ...
    result3 = pd.merge(mlb_df,cities, left_on= "team_ville", right_on= "Metropolitan area")
    
    #df2['W/L%'] = pd.to_numeric(df2['W/L%'])
    result3['W'] = pd.to_numeric(result3['W'])
    result3['L'] = pd.to_numeric(result3['L'])
    result3['Population (2016 est.)[8]'] = pd.to_numeric(result3['Population (2016 est.)[8]'])
    he = ['team','W','L','Metropolitan area','Population (2016 est.)[8]']
    result3 = result3[he]
    result3['W/L'] = result3['W']/(result3['L']+result3['W'])
    result3 = result3.groupby('Metropolitan area').mean().reset_index()
    
    
    population_by_region = result3['Population (2016 est.)[8]'] # pass in metropolitan area population from cities
    win_loss_by_region = result3['W/L'] # pass in win/loss ratio from mlb_df in the same order as cities["Metropolitan area"]
...
def nfl_correlation(): 
    ...
    result4 = pd.merge(nfl_df,cities, left_on= "team_ville", right_on= "Metropolitan area")
    
    
    result4['W'] = pd.to_numeric(result4['W'])
    result4['L'] = pd.to_numeric(result4['L'])
    result4['Population (2016 est.)[8]'] = pd.to_numeric(result4['Population (2016 est.)[8]'])
    he = ['team','W','L','Metropolitan area','Population (2016 est.)[8]']
    result4 = result4[he]
    result4['W/L'] = result4['W']/(result4['L']+result4['W'])
    result4 = result4.groupby('Metropolitan area').mean().reset_index()

    
    population_by_region =result4['Population (2016 est.)[8]'] # pass in metropolitan area population from cities
    win_loss_by_region = result4['W/L'] # pass in win/loss ratio from nfl_df in the same order as cities["Metropolitan area"]
